...
import com.zx.enums.BillCheckMarkEnum;
import com.zx.model.Transaction;
...
public class Java8Test {
    ...
    public void ParallelStreamTest() {
        List<Transaction> listBizBill = JMockData.mock(new TypeReference<>() {
        });
        List<Transaction> listMchBill = JMockData.mock(new TypeReference<>() {
        });
        if (!listBizBill.isEmpty()) {
            listBizBill.parallelStream().forEach(transaction -> {
                String orderNo = transaction.getOrderNo();
                LocalDateTime tradeDateTime = transaction.getTradeDateTime();
                BigDecimal tradeAmount = transaction.getTradeAmount();
                //默认设置为多出
                transaction.setCompareResultMark(BillCheckMarkEnum.REMAINING.getCode());
                transaction.setCompareResultNote(BillCheckMarkEnum.REMAINING.getComment());
                if (!listMchBill.isEmpty()) {
                    for (Transaction mchBill : listMchBill) { // 先以业务账单比对商户账单
                        if (mchBill.getOrderNo().equals(orderNo)) {
                            if (mchBill.getTradeDateTime().toLocalDate().equals(tradeDateTime.toLocalDate())
                                    && mchBill.getTradeAmount().compareTo(tradeAmount) == 0) {
                                transaction.setCompareResultMark(BillCheckMarkEnum.OK.getCode());
                                transaction.setCompareResultNote(BillCheckMarkEnum.OK.getComment());
                                mchBill.setCompareResultMark(BillCheckMarkEnum.OK.getCode());
                                mchBill.setCompareResultNote(BillCheckMarkEnum.OK.getComment());
                            } else {
                                if (!mchBill.getTradeDateTime().toLocalDate().equals(tradeDateTime.toLocalDate())) {
                                    transaction.setCompareResultMark(BillCheckMarkEnum.DIFFERENCE.getCode());
                                    String compareResultNote = "时间不一致";
                                    transaction.setCompareResultNote(compareResultNote);
                                    mchBill.setCompareResultMark(BillCheckMarkEnum.DIFFERENCE.getCode());
                                    mchBill.setCompareResultNote(compareResultNote);
                                }
                                if (mchBill.getTradeAmount().compareTo(tradeAmount) != 0) {
                                    transaction.setCompareResultMark(BillCheckMarkEnum.DIFFERENCE.getCode());
                                    String compareResultNote = "金额不一致";
                                    transaction.setCompareResultNote(compareResultNote);
                                    mchBill.setCompareResultMark(BillCheckMarkEnum.DIFFERENCE.getCode());
                                    if (transaction.getCompareResultNote() != null) {
                                        mchBill.setCompareResultNote(transaction.getCompareResultNote() + "," + compareResultNote);
                                    } else {
                                        mchBill.setCompareResultNote(compareResultNote);
                                    }
                                }
        ...
        if (!listMchBill.isEmpty()) {
            listMchBill.stream().filter(mchBill -> mchBill.getCompareResultMark() == null)
                    .forEachOrdered(mchBill -> {
                        mchBill.setCompareResultMark(BillCheckMarkEnum.OK.getCode());
                        mchBill.setCompareResultNote(BillCheckMarkEnum.OK.getComment());
                    });
        }
